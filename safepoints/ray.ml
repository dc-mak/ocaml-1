...
type 'a nested = Nested of 'a nested list

let rec intersect orig dir (l, _ as hit) (center, radius, scene) =
  match ray_sphere orig dir center radius, scene with
  | l', _ when l' >= l -> hit
  | l', Nested [] -> l', unitise (orig +| l' *| dir -| center)
  | _, Nested scenes -> intersects orig dir hit scenes
and intersects orig dir hit = function
  | [] -> hit
  | scene::scenes -> intersects orig dir (intersect orig dir hit (snd hit, fst hit, scene)) scenes
...
let rec create level c r =
  let obj = c, r, Nested [] in
  if level = 1 then obj else
    let a = 3. *. r /. sqrt 12. in
    let aux x' z' = create (level - 1) (c +| {x=x'; y=a; z=z'}) (0.5 *. r) in
    c, 3. *. r, Nested (List.map thd [obj; aux (-.a) (-.a); aux a (-.a); aux (-.a) a; aux a a])
